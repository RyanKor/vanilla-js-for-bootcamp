...
class Street extends Element {
  ...
  classifyStreet() {
    ...
    console.log(
      `${this.name}, build in ${this.buildYear}, is a ${classification.set(
        this.size
      )} street`
    );
  }
...
function reportStreets(s) {
  ...
  const [totlaLength, avgLength] = calc(s.map((el) => el.length));
  console.log(
    `Our ${s.length} streets have a total length of ${totlaLength} km, with an average of ${avgLength} km.`
  );
  // Classify Sizes
